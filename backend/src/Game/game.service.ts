...
export function userInGame(login: string, worlds: {}) {
    for (const user in worlds) {
        ...
        if (worlds[user].players.player2.user.login === login)
            return user
    }
...
export class matterNode {
    ...
    onHello(callback: (payload: any) => void): void {
        this.eventEmitter.on('hellos', callback);
    }
                ...
                if (this.ready) {
                    ...
                    if (this.ball.position.y < 0) {
                        ...
                        if (this.score.right >= 2) {
                            ...
                            this.eventEmitter.emit('hellos', { resultMatch: resultMatch });
